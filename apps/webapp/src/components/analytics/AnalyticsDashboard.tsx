...
export function AnalyticsDashboard() {
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      bg: 'bg-neon-cyan/10',
      glow: 'shadow-[0_0_12px_-2px_rgba(34,211,238,0.25)]',
    },
    {
      label: 'Конверсий',
      ...
      bg: 'bg-neon-mint/10',
      glow: 'shadow-[0_0_12px_-2px_rgba(52,211,153,0.25)]',
    },
    {
      label: 'CVR',
      ...
      bg: 'bg-neon-amber/10',
      glow: 'shadow-[0_0_12px_-2px_rgba(251,191,36,0.25)]',
    },
  ];

  const eventBreakdown = recentEvents.reduce<Record<string, number>>(
    (acc, e) => {
    ...
    <div className="min-h-screen bg-th-bg relative">
      ...
      <div className="relative z-10">
        ...
        <div className="px-4 pt-3 pb-2">
          <h1 className="text-xl font-bold text-th tracking-tight">
            Аналитика
          </h1>
          <p className="text-sm text-th/35 mt-0.5">
            ...
            {summaryCards.map((card) => {
              ...
              return (
                <div key={card.label} className="glass-card p-4">
                  ...
                  <p className="text-xs text-th/30 mt-0.5">{card.label}</p>
                </div>
              );
            })}
          </div>
        ...
        {(scenarioRuns > 0 || !isLoading) && (
          <div className="px-4 mb-4">
            <div className="glass-card p-4">
              <h3 className="font-semibold text-th text-sm mb-3 flex items-center gap-2">
              ...
              {isLoading ? (
                ...
                <div className="text-center py-4">
                  <PlayCircle className="w-8 h-8 text-th/10 mx-auto mb-2" />
                  <p className="text-th/25 text-xs">
                    Запусти сценарии, чтобы увидеть воронку
                  </p>
                </div>
              ) : (
                <div className="space-y-2.5">
                  <FunnelRow
                    ...
                    value={scenarioRuns}
                    max={scenarioRuns}
                    color="from-brand-500 to-neon-violet"
                  />
                  <FunnelRow
                    icon={
                  ...
                  <div className="pt-1 flex items-center justify-between">
                    <span className="text-xs text-th/25 flex items-center gap-1">
                      <Clock className="w-3 h-3" /> Completion rate
                    </span>
                    <span className="text-sm font-bold text-neon-mint tabular-nums">
                      {scenarioRuns > 0
                        ? Math.round((scenarioCompleted / scenarioRuns) * 100)
                        : 0}
                      %
                    </span>
                  </div>
        ...
        <div className="px-4 mb-4">
          <div className="glass-card p-4">
            <h3 className="font-semibold text-th text-sm mb-3 flex items-center gap-2">
              <BarChart3 className="w-4 h-4 text-brand-400" /> Активность за 30
              дней
            </h3>
            {isLoading ? (
              ...
              <div className="h-40 flex items-center justify-center">
                <p className="text-th/15 text-xs">Нет данных за период</p>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height={160}>
                <AreaChart data={charts?.eventsByDay ?? []}>
                  ...
                  <Tooltip
                    ...
                    cursor={{ stroke: 'rgba(108,92,231,0.2)' }}
                  />
                  <Area
                    type="monotone"
                    dataKey="count"
                  ...
                  .map(({ type, count, label, icon }) => (
                    <div key={type}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm text-th/50 flex items-center gap-1.5">
                          <span>{icon}</span> {label}
                        </span>
                        <span className="text-xs font-semibold text-th tabular-nums">
                          {count}
                        </span>
                      </div>
        ...
        {(charts?.linksByClicks?.length ?? 0) > 0 && (
          <div className="px-4 mb-4">
            <div className="glass-card p-4">
              <h3 className="font-semibold text-th text-sm mb-3 flex items-center gap-2">
                <Link2 className="w-4 h-4 text-neon-cyan" /> Топ ссылок по
                кликам
              </h3>
              <ResponsiveContainer
                width="100%"
                height={Math.min(
                  (charts?.linksByClicks?.length ?? 1) * 36,
                  180,
                )}
              >
        ...
        {recentEvents.length > 0 && (
          <div className="px-4 mb-4">
            <div className="glass-card p-4">
              <h3 className="font-semibold text-th text-sm mb-3 flex items-center gap-2">
                <Activity className="w-4 h-4 text-neon-mint" /> Последние
                события
              </h3>
              <div className="space-y-1">
                {recentEvents.slice(0, 10).map((event) => (
                  <div
                    key={event.id}
                    className="flex items-center gap-3 py-2 border-b border-th-border/[0.04] last:border-0"
                  >
                    <span className="text-base flex-shrink-0">
                      {EVENT_ICONS[event.type] ?? '📊'}
                    </span>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-th/60 truncate">
                        {EVENT_LABELS[event.type] ?? event.type}
                      </p>
                    </div>
                    <span className="text-[10px] text-th/20 flex-shrink-0 tabular-nums font-mono">
                      {new Date(event.createdAt).toLocaleTimeString('ru', {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </span>
        ...
        {!isLoading && !data && (
          <div className="px-4">
            <div className="glass-card p-8 text-center">
              <BarChart3 className="w-12 h-12 text-th/10 mx-auto mb-3" />
              <p className="text-th/40 text-sm">
                Аналитика появится после первых действий
              </p>
              <p className="text-th/20 text-xs mt-1">
                Создай трекинг-ссылку и запусти сценарий
              </p>
            </div>
          </div>
        )}
...
function FunnelRow({
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs text-th/40 flex items-center gap-1.5">
          {icon}
          {label}
        </span>
        <span className="text-xs font-semibold text-th tabular-nums">
          {value} <span className="text-th/25">({Math.round(pct)}%)</span>
        </span>
      </div>
      <div className="h-2 bg-th-border/[0.04] rounded-full overflow-hidden">
        <motion.div
