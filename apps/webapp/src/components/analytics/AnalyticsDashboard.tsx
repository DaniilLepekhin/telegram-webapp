    ...
    <div className="min-h-screen bg-th-bg relative">
      ...
      <div className="relative z-10">
        ...
        <div className="px-4 pt-3 pb-2">
          <h1 className="text-xl font-bold text-th tracking-tight">Аналитика</h1>
          <p className="text-sm text-th/35 mt-0.5">Метрики в реальном времени</p>
        </div>
            ...
            <div className="glass-card p-4">
              ...
              {isLoading ? (
                ...
                <div className="text-center py-4">
                  ...
                  <p className="text-th/25 text-xs">Запусти сценарии, чтобы увидеть воронку</p>
                </div>
              ) : (
                ...
                {breakdownList.slice(0, 8).map(({ type, count, label, icon }) => (
                  <div key={type}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-semibold text-th tabular-nums">{count}</span>
                    </div>
            ...
            <div className="glass-card p-4">
              ...
              <div className="space-y-1">
                {recentEvents.slice(0, 10).map((event) => (
                    <div key={event.id} className="flex items-center gap-3 py-2 border-b border-th-border/[0.04] last:border-0">
                    <span className="text-base flex-shrink-0">{EVENT_ICONS[event.type] ?? '📊'}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-th/60 truncate">{EVENT_LABELS[event.type] ?? event.type}</p>
                    </div>
        ...
        {!isLoading && !data && (
          <div className="px-4">
            <div className="glass-card p-8 text-center">
              ...
              <p className="text-th/40 text-sm">Аналитика появится после первых действий</p>
              <p className="text-th/20 text-xs mt-1">Создай трекинг-ссылку и запусти сценарий</p>
            </div>
          </div>
        )}
      </div>
...
function FunnelRow({ icon, label, value, max, color }: { icon: React.ReactNode; label: string; value: number; max: number; color: string }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs text-th/40 flex items-center gap-1.5">{icon}{label}</span>
        <span className="text-xs font-semibold text-th tabular-nums">{value} <span className="text-th/25">({Math.round(pct)}%)</span></span>
      </div>
