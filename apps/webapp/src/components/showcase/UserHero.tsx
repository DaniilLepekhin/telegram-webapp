...
export function UserHero({ user }: UserHeroProps) {
  ...
  const progress =
    levelRange > 0
      ? Math.min(((xp - currentLevelXp) / levelRange) * 100, 100)
      : 100;
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-xs text-th/40 font-medium">
                  Ур. {level}
                </span>
            ...
            {streak > 0 && (
              <div className="flex items-center gap-1.5 glass px-3 py-2 rounded-xl">
                ...
                <span className="text-sm font-bold text-th tabular-nums">
                  {streak}
                </span>
              </div>
            )}
          ...
          {isFreshAuth && (
            <div className="mt-4">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-[10px] text-th/25">
                  {xpToNext.toLocaleString('ru-RU')} до{' '}
                  {LEVEL_NAMES[level + 1] ?? 'макс'}
                </span>
              </div>
              <div className="h-1.5 bg-th-border/[0.04] rounded-full overflow-hidden">
                <motion.div
                  ...
                  style={{
                    background:
                      'linear-gradient(90deg, #6c5ce7, #a855f7, #22d3ee)',
                  }}
                  initial={{ width: 0 }}
