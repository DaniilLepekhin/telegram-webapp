...
export function GamificationPage() {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return r.data as GamificationStats;
    },
    enabled,
    staleTime: 5 * 60_000,
  ...
  const { data: leaderboard, isLoading: lbLoading } = useQuery({
    ...
    queryFn: async () => {
      const r = await api.getLeaderboard(20);
  ...
  const awardMutation = useMutation({
    mutationFn: ({
      ...
      reason,
      actionType,
    }: { amount: number; reason: string; actionType: string }) =>
    ...
    onSuccess: (data: unknown) => {
      ...
      const result = (
        data as {
          data?: { leveledUp?: boolean; newLevel?: number; newXp?: number };
        }
      )?.data;
      if (result?.leveledUp) {
        toast.success(
          `Уровень повышен! Теперь ты ${LEVEL_NAMES[result.newLevel ?? 0]}`,
        );
      } else {
        toast.success(
          `+${(result?.newXp ?? 0) - (stats?.xp ?? 0)} XP заработано!`,
        );
      }
    },
  });
  ...
  const quickActions = [
    {
      label: '+25 XP — Открыл кейс',
      amount: 25,
      reason: 'Просмотр кейса',
      type: 'page_view',
    },
    {
      label: '+50 XP — Ежедневный бонус',
      amount: 50,
      reason: 'Ежедневный вход',
      type: 'daily_login',
    },
    {
      label: '+100 XP — Создал ссылку',
      amount: 100,
      reason: 'Создание трекинг-ссылки',
      type: 'link_created',
    },
  ];
      ...
      <div className="relative z-10 pb-4">
        ...
        <div className="px-4 pt-4 pb-3">
          ...
          <p className="text-sm text-th/35 mt-0.5">
            XP, уровни, достижения, лидерборд
          </p>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'stats' && (
            ...
            >
                ...
                <div className="glass-card p-5 relative overflow-hidden">
                  ...
                  <div className="relative flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <div>
                          <p className="font-bold text-th text-lg">
                            {LEVEL_NAMES[level] ?? 'Новичок'}
                          </p>
                  ...
                  <div className="mt-4">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-xs text-th/30">
                        {xpToNext > 0
                          ? `+${xpToNext} до ${LEVEL_NAMES[level + 1] ?? 'макс'}`
                          : 'Макс уровень'}
                      </span>
                    </div>
                  ...
                  <div className="flex gap-6 mt-4 pt-4 border-t border-th-border/5">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-th">
                        {unlockedIds.size} / {allAchievements?.length ?? 10}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {isFreshAuth && (
                <div className="px-4 mb-4">
                  <p className="text-xs text-th/40 uppercase tracking-wider mb-2">
                    Быстрые действия
                  </p>
                    ...
                    {quickActions.map((action) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm text-th/80 font-medium">
                            {action.label}
                          </span>
                        </div>
              ...
              {activity && activity.length > 0 && (
                <div className="px-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-xs text-th/40 uppercase tracking-wider">
                      Недавняя активность
                    </p>
                  </div>
                  <div className="space-y-2">
                    {activity.slice(0, 5).map((e) => (
                      ...
                      >
                        <div>
                          <p className="text-xs text-th/70 font-medium">
                            {e.reason}
                          </p>
                ...
                {(allAchievements ?? []).map((ach) => {
                  ...
                  return (
                    ...
                    >
                      {isUnlocked && (
                        <div
                          className={cn(
                            ...
                            RARITY_COLORS[
                              ach.rarity as keyof typeof RARITY_COLORS
                            ],
                          )}
                        />
                      )}
                      ...
                      <p className="font-semibold text-th text-xs leading-tight">
                        {ach.name}
                      </p>
                      <p className="text-[10px] text-th/40 mt-0.5 line-clamp-2">
                        {ach.description}
                      </p>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span
                          className={cn(
                            ...
                            isUnlocked
                              ? `bg-gradient-to-r ${RARITY_COLORS[ach.rarity as keyof typeof RARITY_COLORS]} text-th-invert`
                              : 'bg-th-border/5 text-th/30',
                          )}
                        >
                          {
                            RARITY_LABELS[
                              ach.rarity as keyof typeof RARITY_LABELS
                            ]
                          }
                        </span>
          ...
          {activeTab === 'leaderboard' && (
            <motion.div
              ...
              className="px-4"
            >
              <p className="text-xs text-th/40 uppercase tracking-wider mb-3">
                Топ по XP
              </p>
                  ...
                  {(leaderboard ?? []).map((entry, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-th/40">
                          Ур. {entry.level} {LEVEL_NAMES[entry.level] ?? ''}
                        </p>
                      </div>
