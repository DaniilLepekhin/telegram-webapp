...
function verifyLavaSignature(
  ...
  if (!secret) {
    logger.error(
      'LAVA_WEBHOOK_SECRET is not set — rejecting webhook (configure before going live)',
    );
...
function verifyCloudPaymentsSignature(
  ...
  if (!secret) {
    logger.error(
      'CP_WEBHOOK_SECRET is not set — rejecting webhook (configure before going live)',
    );
    return false; // fail closed
  }
  if (!signature) return false;
...
export const subscriptionsModule = new Elysia({ prefix: '/subscriptions' })
  ...
  .post(
    ...
    (async (ctx: any) => {
      const { body, user, set } = ctx as {
        body: { plan: string; trial?: boolean; provider?: string };
        user: { id: string };
        set: { status: number };
      };
      const userId = user.id;
      ...
      if (!plan) {
        ...
        return {
          success: false,
          error: { code: 'INVALID_PLAN', message: 'Неверный план' },
        };
      }
      ...
      const expiresAt = new Date(
        now.getTime() + (isTrial ? plan.trialDays : 30) * 24 * 60 * 60 * 1000,
      );
      ...
      const provider: PaymentProvider =
        (body.provider as PaymentProvider | undefined) ?? 'lava';
      const paymentUrl = !isTrial
        ? generatePaymentUrl(provider, plan.price, userId)
        : null;

      logger.info(
        { userId, plan: body.plan, isTrial, subscriptionId: sub.id },
  ...
  .post('/cancel', (async (ctx: any) => {
    ...
    return {
      success: true,
      data: {
        message: 'Подписка отменена. Доступ сохраняется до конца периода.',
      },
    };
  }) as any)
  ...
  .post(
    ...
    async ({ params, set, request }) => {
      ...
      try {
        ...
        return {
          success: false,
          error: { code: 'INVALID_BODY', message: 'Невалидный JSON' },
        };
      }
      ...
      async function activateSubscription(
        ...
        if (!sub) {
          logger.warn(
            { orderId, provider },
            'Webhook: subscription not found for orderId',
          );
          return;
        }
        ...
        logger.info(
          { orderId, userId: sub.userId, provider },
          'Subscription activated via webhook',
        );
      }

      if (provider === 'lava') {
        const { orderId, status, amount } = payload as {
          orderId?: string;
          status: string;
          amount?: number;
        };
      ...
      if (provider === 'cloudpayments') {
        const { InvoiceId: orderId, Status: status } = payload as {
          InvoiceId?: string;
          Status: string;
        };
        if (status === 'Completed') {
