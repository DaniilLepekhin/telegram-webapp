...
export const scenarioIdEnum = pgEnum('scenario_id', [
  'ecom',
  'club',
  'service',
  'education',
...
export const trackingLinks = pgTable(
  'tracking_links',
  {
    ...
    abTestGroups:
      jsonb('ab_test_groups').$type<
        Array<{ name: string; weight: number; url: string }>
      >(),
...
export const xpHistory = pgTable(
  ...
  (t) => [index('xp_history_user_id_idx').on(t.userId)],
);
...
export const quests = pgTable('quests', {
  ...
  steps: jsonb('steps')
    .$type<Array<{ id: string; title: string; target: number }>>()
    .notNull(),
...
export const trackingClicksRelations = relations(trackingClicks, ({ one }) => ({
  link: one(trackingLinks, {
    fields: [trackingClicks.linkId],
