...
function AppContent() {
  ...
  // useEffect для автоматической прокрутки к верху при смене страницы
  useEffect(() => {
    // Прокручиваем к верху страницы при каждом изменении currentPage
    // Используем двойную задержку для гарантии выполнения после рендеринга
    setTimeout(() => {
      window.scrollTo({ top: 0, behavior: 'instant' });
    }, 50);
    
    // Логируем изменение страницы для отладки
    console.log('📄 Страница изменена на:', currentPage);
  }, [currentPage]);
  ...
  useEffect(() => {
    ...
    const navigateTo = (page: Page) => {
      ...
      // Принудительно прокручиваем к верху сразу после установки страницы
      setTimeout(() => {
        window.scrollTo({ top: 0, behavior: 'instant' });
      }, 10);
    };
  ...
  const goBack = () => {
    ...
    // Простая проверка: если мы на главной странице, показываем уведомление
    if (currentPage === 'main') {
    ...
    if (navigationHistory.length > 1) {
      ...
      // Принудительно прокручиваем к верху
      setTimeout(() => {
        window.scrollTo({ top: 0, behavior: 'instant' });
      }, 10);
    } else {
      // Если история пуста или некорректна, возвращаемся на главную
      console.log('🏠 История некорректна, возвращаемся на главную');
      setCurrentPage('main');
